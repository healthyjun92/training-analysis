...
class AIAssistant extends HTMLElement {
  ...
  async generateResponse(question) {
    ...
    setTimeout(() => {
      ...
      if (question.includes('분석') || question.includes('어때')) {
        ...
        response = `달리기는 현재 ${totals.run.dist}km를 소화하셨네요. 관절 부상을 방지하기 위해 훈련 후에는 반드시 리커버리 런을 섞어주세요.`;
      } else {
  ...
  getPerformanceAnalysis(totals, activities) {
    ...
    return `최근 데이터를 분석한 결과, ${disciplineKo[mainDiscipline]} 훈련 비중이 가장 높습니다. 균형 있는 발전을 위해 상대적으로 부족한 종목의 훈련 시간을 15% 정도 늘리는 것을 추천합니다. 충분한 휴식도 잊지 마세요.`;
  }
  ...
  getTrainingRecommendation(totals) {
    ...
    return `오늘은 가장 훈련량이 적은 **${disciplineKo[leastTrained]}** 훈련을 추천합니다. 약 45분 정도의 지속주(Tempo) 훈련이 현재 스케줄에 가장 적합해 보입니다.`;
  }
